...
```sh
./scripts/release_macos.sh 0.16.1-local --local
open dist/macos/MachBoost-0.16.1-local-arm64.dmg
```
...
./scripts/release_macos.sh 0.16.1
./scripts/publish_macos_release.sh 0.16.1 ./release-notes/0.16.1.md
```
...
signed Sparkle appcast. The publisher requires an existing `v0.16.1` tag, an
