...
```sh
./scripts/release_macos.sh 0.15.3-local --local
open dist/macos/MachBoost-0.15.3-local-arm64.dmg
```
...
./scripts/release_macos.sh 0.15.3
./scripts/publish_macos_release.sh 0.15.3 ./release-notes/0.15.3.md
```
...
signed Sparkle appcast. The publisher requires an existing `v0.15.3` tag, an
